...
1. Use the **.NET** workflow when the repository contains `.cs`, `.csproj`, `.sln`, `.slnx`, or other .NET project files, or when the user explicitly asks for C# or .NET guidance. Follow [references/dotnet.md](https://github.com/github/awesome-copilot/blob/main/skills/microsoft-agent-framework/references/dotnet.md).
2. Use the **Python** workflow when the repository contains `.py`, `pyproject.toml`, `requirements.txt`, or the user explicitly asks for Python guidance. Follow [references/python.md](https://github.com/github/awesome-copilot/blob/main/skills/microsoft-agent-framework/references/python.md).
...
## DataNative-specific guidance

This project uses Microsoft Agent Framework 1.0.0-rc4 with:

- **Package versions**: `Microsoft.Agents.AI.OpenAI` 1.0.0-rc4, `Microsoft.Agents.AI.Workflows` 1.0.0-rc4
- **A2A Protocol**: `A2A` 0.3.4-preview, `Microsoft.Agents.AI.A2A` 1.0.0-preview
- **DevUI**: `Microsoft.Agents.AI.DevUI` 1.0.0-preview at `/devui`
- **FORBIDDEN**: `[KernelFunction]` attributes — use `[Description]` attributes instead
- **FORBIDDEN**: `Microsoft.SemanticKernel` — fully replaced by Microsoft Agent Framework
- **AI providers**: GitHubModels, Ollama, Gemini via shared `IChatClient` factory
- **ToolRegistry**: `AITool`-based plugin system replacing `KernelPlugin`
- **Workflows**: `AgentWorkflowBuilder` for Single, Sequential, Parallel, GroupChat, Handoff
- **Agents location**: `src/Agents/DataNative.Agent.Orchestrator/`
- **A2A servers**: `DataNative.BrowserAgent` (Playwright) and `DataNative.KnowledgeSearchAgent`
...
- [.NET reference](https://github.com/github/awesome-copilot/blob/main/skills/microsoft-agent-framework/references/dotnet.md)
- [Python reference](https://github.com/github/awesome-copilot/blob/main/skills/microsoft-agent-framework/references/python.md)
